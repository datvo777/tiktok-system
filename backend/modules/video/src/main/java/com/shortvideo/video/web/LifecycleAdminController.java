...
public class LifecycleAdminController {
    ...
    public ResponseEntity<Void> quarantine(@PathVariable String videoId, @RequestBody(required = false) LifecycleDtos.ReasonRequest request) {
        videoService.quarantine(videoId, request == null ? null : request.reason());
    ...
    public ResponseEntity<Void> restore(@PathVariable String videoId) {
        videoService.restoreFromQuarantine(videoId);
        return ResponseEntity.noContent().build();
    }
    ...
    public ResponseEntity<Void> remove(@PathVariable String videoId, @RequestBody(required = false) LifecycleDtos.ReasonRequest request) {
        videoService.remove(videoId, request == null ? null : request.reason());
        return ResponseEntity.noContent().build();
    }
    ...
    public ResponseEntity<Void> reprocess(@PathVariable String videoId) {
        videoService.reprocess(videoId);
        return ResponseEntity.noContent().build();
    }
}
